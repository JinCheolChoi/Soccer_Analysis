...
bankrole=c()
win.or.loss.n=0
bankrole[1]=initial.bankrole
final.decision$Result=0
for(bankrole.n in 1:bankrole.n.length){ #1
  if(is.na(match(bankrole.n, unique(final.decision$Time)))){
    if(bankrole.n == 1){
      bankrole[bankrole.n]=initial.bankrole
    }else{
      bankrole[bankrole.n]=bankrole[bankrole.n-1]
    }
  }else{
    #bet.n=3
    if(bankrole.n > 1){
      if(bankrole[bankrole.n-1] <= 0){
        bankrole[bankrole.n]=0
      }
    }
    target.data.index=which(analysis.data$Time==bankrole.n)
    target.data=analysis.data[target.data.index,]
    
    target.decision.index=which(final.decision$Time==bankrole.n)
    target.decision=final.decision[target.decision.index,]
    
    ## invest money
    temp.bankrole=c()
    if(bankrole.n == 1){
      temp.bankrole=bankrole[bankrole.n]
    }else{temp.bankrole=bankrole[bankrole.n-1]}
    temp.invest=c()
    ...
    for(ind.bet.n in nrow(target.decision):1){ #3
      ...
      if(first.match == TRUE && second.match == TRUE){ #4
        ...
        ## count win
        final.decision$Result[nrow(final.decision)+1-win.or.loss.n]=1
      }else{ ### when I lose a bet
        ## count lose
        final.decision$Result[nrow(final.decision)+1-win.or.loss.n]=0
      } #4
      
    } #3
    
    bankrole[bankrole.n]=temp.bankrole
  }
