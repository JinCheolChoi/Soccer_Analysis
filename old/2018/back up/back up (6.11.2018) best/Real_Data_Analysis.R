...
plot(final.bankrole$capital[1:40], type='o')
plot(final.bankrole$capital[41:99], type='o')
final.decision[which(final.decision$result.mat==1),]
################
# logistic model
################
...
pro.compute = function(max.loss.gain.prob, joint.odds.scalar, winning.prob){
  loss.gain.prob = c()
  k.space = seq(0.0001, 0.9999, by=0.0001)
  for(k in k.space){
