...
for(League in Leagues){
  ...
  #*************************
  Over_Under_Score_Odds=c()
  for(Year in Years){
    #Year=2021
    Temp_Over_Under_Score_Odds=fread(paste0(data.dir_2, "/", League, "/", Year, ".csv"))
    if(!is.na(as.Date(Temp_Over_Under_Score_Odds$Date[1], format="%Y-%m-%d"))){
      Temp_Over_Under_Score_Odds[, Date:=as.Date(Date, format="%Y-%m-%d")]
    }else if(!is.na(as.Date(Temp_Over_Under_Score_Odds$Date[1], format="%m/%d/%Y"))){
      Temp_Over_Under_Score_Odds[, Date:=as.Date(Date, format="%m/%d/%Y")]
    }
