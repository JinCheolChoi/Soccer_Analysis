...
#
#****************
# Kelly_Criterion=function(p, b){ # 100000, Exact, 1, Ind, "championship", "league-two", "laliga"
#                                 # Kelly, Exact, 1, Ind, "championship", "league-two", "laliga"
#   Kelly=(p)-(1-p)/(b/2-1)       # 100000, Implied, 1, Uni
#   
#   return(Kelly/150)
# }
Kelly_Criterion=function(p, b){ # 10000, Exact, 1, Uni, "championship", "league-two", "laliga"
                                # Kelly, Implied, 1, Uni, "championship", "league-two", "laliga"
  Kelly=(p)-(1-p)/(b/6-1)       
  
  return(Kelly/150)
}
# Kelly_Criterion=function(p, b){ # Kelly, Exact, 1, Ind
#   Kelly=(p)-(1-p)/(b/2-1)
# 
#   return(log(Kelly)/10)
# }
# Kelly_Criterion=function(p, b){ # Kelly, Implied, 3, Uni
#   Kelly=(p)-(1-p)/(b/1.1-1)     # Kelly, Implied, 3, ind
# 
#   return(log10(1+log10(1+Kelly))/10)
# }


# Kelly_Criterion=function(p, b, c=1){
#   Kelly=(p)-(1-p)/(b*c-1)
# 
#   return(Kelly)
# }
# 
# 1/(0.2000000*1.95)
# (0.2000000*1.95*2.56-1)/
# (1.95*0.4-1)
# Kelly_Criterion(0.2000000, 1.95, 0.4)
# 
# Kelly_Criterion(p=0.5, 2, c=1)
# Out=c()
# C=seq(-5, 5, by=0.01)
# for(i in 1:length(C)){
#  Out[i]=Kelly_Criterion(p=0.5, 3, c=C[i])
# }
# C[which(Out>10)]
# Kelly_Criterion(p=0.5, 3, c=0.32)
# plot(C, Out)
...
#
#********************
Total_Goals_Kelly_Calculator=function(Game_URL, Country, League, Prob_Estimate, Chosen_Profit_Criteria, ...){
  # game URL
  ...
  #****************
  # Kelly score
  Over_Under_Score_Odds=Compute_Kelly_Scores(Over_Under_Score_Odds, Results_Data, League, Prob_Estimate, Chosen_Profit_Criteria)
...
#
#*********************
Compute_Kelly_Scores=function(Over_Under_Score_Odds, Results_Data, League_Text, Prob_Estimate, Chosen_Profit_Criteria, ...) {
