...
#
#****************
Kelly_Criterion=function(p, b){(p)-(1-(p))/(b*2-1)}

#******************************************
#
# Kelly_Criterion_Calculator_Negative_Binom
#
#******************************************
...
#**************************************
#
# Kelly_Criterion_Calculator_Exact_Dist
#
#**************************************
Kelly_Criterion_Calculator_Exact_Dist=function(x, Results_Data){
  ...
  return(
    data.table(
      ...
      # `Over 0.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<0.5)), as.numeric(x["Over 0.5"])), # Date is important to take into account only data prior to the game
      # `Under 0.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<0.5), as.numeric(x["Under 0.5"])),
      # 
      # `Over 1.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<1.5)), as.numeric(x["Over 1.5"])),
      # `Under 1.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<1.5), as.numeric(x["Under 1.5"])),
      # 
      # `Over 2.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<2.5)), as.numeric(x["Over 2.5"])),
      # `Under 2.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<2.5), as.numeric(x["Under 2.5"])),
      # 
      # `Over 3.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<3.5)), as.numeric(x["Over 3.5"])),
      # `Under 3.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<3.5), as.numeric(x["Under 3.5"])),
      # 
      # `Over 4.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<4.5)), as.numeric(x["Over 4.5"])),
      # `Under 4.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<4.5), as.numeric(x["Under 4.5"])),
      # 
      # `Over 5.5`=Kelly_Criterion((1-mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<5.5)), as.numeric(x["Over 5.5"])),
      # `Under 5.5`=Kelly_Criterion(mean(Results_Data[Exact_Date<x["Date"], Total_Goal]<5.5), as.numeric(x["Under 5.5"]))
    )
  )
}
...
#***********************************
#
# Kelly_Criterion_Calculator_Poisson
#
#***********************************
...
#
#********************
Total_Goals_Kelly_Calculator=function(Game_URL, Country, League, Kelly_Method, Chosen_Profit_Criteria, ...){
  # game URL
  ...
  while(Target_Options_N==0){
    ...
    while(length(Headings)==0){
      ...
      if(N_Try==5){
        print("Country : [", Country, "], League : [", League, "], re-open the server")
...
#
#*********************
Compute_Kelly_Scores=function(Over_Under_Score_Odds, Results_Data, League_Text, Kelly_Method, Chosen_Profit_Criteria, ...) {
  # Kelly score
  #Kelly_Method="Poisson" # Poisson, Exact_Dist, Negative_Binom
  if (Kelly_Method=="Poisson") {
    Over_Under_Score_Kelly = do.call(
      rbind.data.frame,
      apply(Over_Under_Score_Odds, 1, function(x)
        Kelly_Criterion_Calculator_Poisson(x, Results_Data[League==League_Text, ]))
    )
  } else if (Kelly_Method=="Exact_Dist") {
    Over_Under_Score_Kelly = do.call(
      rbind.data.frame,
      apply(Over_Under_Score_Odds, 1, function(x)
        Kelly_Criterion_Calculator_Exact_Dist(x, Results_Data[League==League_Text, ]))
    )
  } else if (Kelly_Method=="Negative_Binom") {
    Over_Under_Score_Kelly = do.call(
      rbind.data.frame,
      apply(Over_Under_Score_Odds, 1, function(x)
        Kelly_Criterion_Calculator_Negative_Binom(x, Results_Data[League==League_Text, ]))
    )
  }
  ...
  Over_Under_Score_Exact_Prob = do.call(rbind.data.frame,
                                        apply(Over_Under_Score_Odds, 1, function(x)
                                          Exact_Prob(x, Results_Data[League==League_Text, ])))
