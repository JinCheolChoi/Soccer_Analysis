...
Total_Goals_Kelly_Calculator=function(Game_URL, Country, League, Kelly_Method, Chosen_Profit_Criteria, ...){
  ...
  Over_Under_Score_Var=Over_Under_Score_Exact_Prob %>% 
    ...
    dplyr::select(Date, League, Home_Team, Away_Team, 
                  paste0(Over_Under_Categories, " Var"))
  ...
  for(i in 1:nrow(Over_Under_Score_Profit)){
    if(Chosen_Profit_Criteria==1){
      ...
      Chosen_Profit=unlist(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories])[which(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories]>0)][which.max(unlist(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories])[which(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories]>0)])]
    }else if(Chosen_Profit_Criteria==2){
      #**************************************************
      # Chosen_Profit Criterion - (2) : by minimum profit
      Chosen_Profit=unlist(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories])[which(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories]>0)][which.min(unlist(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories])[which(Over_Under_Score_Profit[i, .SD, .SDcols=Over_Under_Categories]>0)])]
